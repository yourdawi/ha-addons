...
## Network and ports
The add-on exposes Pi-hole's standard ports by default:
Default ports:
- 53/tcp, 53/udp – DNS (enabled by default)
- 80/tcp – HTTP Web UI (enabled by default)
- 443/tcp – HTTPS (self-signed cert generated by FTL, enabled by default)
...
Note: Port 53 commonly conflicts with host/system DNS. Ensure no other service is bound to port 53. If your HA host already uses port 80 or 443, adjust the mapped ports in the add-on configuration.

The Home Assistant "Open Web UI" button links to `/admin/`.
